...
## 2026-09-20 CORE-01.T07 bounded cancellation retirement

PR #829 merged as `c2b400bf2495ed377683c536f0ab5623e57a711d` from exact code head `948b5cc523d3abaed5d38ce18e962a68ac6b4fe2` (tree `f92fa023818a2feb5902b534f71caef73dcd04c8`). The cancellation registry now binds a cancellation ID to its owning worker generation, so freeing bounded capacity cannot make a late old-generation cancellation current again. Incoming cancellation retains a backpressured outbound `CancelAck` obligation until retry and drain, while locally initiated cancellation becomes dispatch-inactive after its `Cancel` is queued and cannot retire until the peer ACK is consumed. Reliable, progress and artifact events carry the exact registration and cancelled/stale queued payload is discarded rather than emitted.

The capacity-one regressions cycle eight incoming completions and eight locally initiated cancellations without exhausting the registry. They also cover unrelated reserved-control ACK backpressure, queued-work suppression, unsolicited ACK rejection, duplicate ACK idempotence and simultaneous inbound/outbound ACK obligations. Pinned local targeted `intent-ipc` tests, formatting and strict workspace Clippy passed before publication.

Exact-head CI run 35478190118 passed formatting, strict Clippy, workspace tests, doctests, fuzz-target compilation, architecture/conformance and native Ubuntu 24.04, Windows 2025 and macOS 15 jobs. Artifact 10594868214 has SHA-256 `2594acae8689ae15209fb7c54929cf1a60506e4115d666a21c69995cc64feed5`, pins synthetic merge `bd3d39b41953e0a17c8c5d63e55f8a08304afae0`, and retains unchanged lockfile hashes. Instrumented parser fuzz run 35478190127 passed `frame_decoder`, `control_envelope` and `record_codec`. Fresh Codex review completed on the exact code head with no major issues (comment 5746345633), and all three inline review threads were resolved before merge.

This is not CORE-01.T07 acceptance. The live issue still requires real spawned-worker cancellation while bulk transport is blocked, stop-deadline/control-latency qualification, trusted durable-dispatch integration including #141 lease-first cancellation/persistence, stale-epoch/in-flight-effect end-to-end reconciliation, byte-budget qualification and independent whole-task acceptance. Issue #127 remains open, `--require-accepted` is unchanged, and production readiness is not established.
