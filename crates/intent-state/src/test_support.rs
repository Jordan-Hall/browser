use crate::{
    ArtifactMetadata, ArtifactScope, DurableOperation, DurableOperationState, NewArtifact,
    NewDurableOperation, OperationTransition, StateStore,
};
use intent_contracts::{
    BoundedText, ContentHash, OperationAttemptId, OperationId, SchemaVersion, UnixTimestampMicros,
};
use std::{
    error::Error,
    fs,
...
pub fn artifact(
    ...
    Ok(store.store_artifact(
        root,
        new_artifact(number, scope)?,
        &mut Cursor::new(b"retained bytes"),
    )?)
}

pub fn new_artifact(number: u64, scope: &ArtifactScope) -> TestResult<NewArtifact> {
    Ok(NewArtifact {
