...
pub(crate) fn check() -> ConformanceResult<Vec<ConformanceCheck>> {
    ...
    let executions: Vec<Value> = serde_json::from_str(include_str!(
        "../../../fixtures/core/execution-records-v1.json"
    ))?;
    for fixture in fixtures {
        let kind = fixture.family;
        let family = kind.record_family()?;
        let mut documents = vec![&fixture.full, &fixture.minimal];
        if kind == CoreRecordKind::Operation {
            documents.extend(executions.iter());
        }
        for (index, document) in documents.into_iter().enumerate() {
            ...
            if index != 1 && serde_json::from_slice::<Value>(&encoded)? != *document {
